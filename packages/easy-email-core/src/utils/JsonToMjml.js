...
export function JsonToMjml(options) {
    const { data, idx = 'content', context = data, mode = 'production', dataSource = {}, } = options;
    if ((isBoolean(data?.data?.hidden) && data?.data?.hidden) ||
    ...
    if (block.render) {
        ...
        const transformData = isValidElement(transformBlockData)
            ...
            : transformBlockData;
        att['css-class'] = [
            ...new Set(classnames(isTest && getPreviewClassName(idx, data.type), transformData?.['attributes']?.['css-class']).split(' ')),
        ].join(' ');
        return JsonToMjml({
            data: {
                ...transformData,
        ...
        case BasicType.PAGE:
           ...
           <mj-style inline="inline">.mjml-body { width: ${data.attributes.width || '600px'}; margin: 0px auto; }</mj-style>`
            ...
            const styles = value.headStyles
                ?.map((style) => `<mj-style ${style.inline ? 'inline="inline"' : ''}>${style.content}</mj-style>`)
                .join('\n') || '';
...
export function renderPlaceholder(type) {
    let text = '';
...
//# sourceMappingURL=JsonToMjml.js.map