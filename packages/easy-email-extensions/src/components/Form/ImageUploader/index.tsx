...
export function ImageUploader(props: ImageUploaderProps) {
  ...
  const content = useMemo(() => {
    ...
    if (!props.value) {
      return (
        <div className={styles['upload']} onClick={onUpload} id='uploadDiv'>
          <IconPlus />
          <div>Upload</div>
        </div>
      );
    ...
    return (
      <div className='hover-pointer'>
        <div className={styles['item']} onClick={() => { onRemove(); onUpload(); }}>
          <div className={classnames(styles['info'])}>
            <img src={props.value} />
            <div className={styles['btn-wrap']} title='Change' />
          </div>
        </div>
      </div>
    );
