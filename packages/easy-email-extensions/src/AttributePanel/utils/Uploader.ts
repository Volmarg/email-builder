...
export class Uploader {
    ...
    if (accept) {
      ...
      for (const file of files) {
        if (file.type.indexOf(fileType) !== 0) {
          let msg = 'This file-type is not allowed!'
  ...
  private checkSize(files: File[]) {
    ...
    for (const file of files) {
      let currKiloBytes = file.size / 1000;
      if (options.minSize && file.size < options.minSize) {
        let minKiloBytes = options.minSize / 1000;
        let msg = `Uploaded file-size is too small (${currKiloBytes}kB). Min: ${minKiloBytes}kB.`;
      ...
      if (options.maxSize && file.size > options.maxSize) {
        let maxKiloBytes = options.maxSize / 1000;
        let msg = `Uploaded file-size is too big (${currKiloBytes}kB). Max: ${maxKiloBytes}kB.`;
