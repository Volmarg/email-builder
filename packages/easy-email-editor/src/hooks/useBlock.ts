...
import { AwnStyles } from '@demo';
import { AwnCustomStyles } from '@demo';
  ...
  const addBlock = useCallback(
    (params: {
      ...
      if (!fixedBlock?.validParentType.includes(parent.type)) {
          ...
          let msg = `
            <b>${block.type.replaceAll('_', ' ')}</b> 
            cannot be used inside <b>${parentBlock.type}</b>, only inside: 
            <b>${block.validParentType.join(', ').replaceAll('_', ' ')}</b>.
         `;
